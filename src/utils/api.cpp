...
void EteraAPI::retranslateUi() {
    OK_MESSAGE = tr("OK");
    JSON_PARSE_ERROR = tr("Ошибка разбора JSON");
    MALFORMED_LINK_URL = tr("Неподдерживаемый URL объекта Link");
    UNSUPPORTED_LINK_METHOD = tr("Неподдерживаемый метод объекта Link");
    FILE_OPEN_ERROR = tr("Ошибка открытия файла");
    TIMED_OUT_ERROR = tr("Таймаут сетевой операции");
}
//----------------------------------------------------------------------------------------------
...
bool EteraAPI::startRequest(const QNetworkRequest& request, EteraRequestMethod method, const QString& data, QIODevice* io) {
    ...
    if(g_api_enabled == false)
        return setLastError(1, tr("API не инициализировано"));
...
void EteraAPI::getToken(const QString& auth_code) {
    ...
    QString oauth_base = tr("oauth.yandex.ru");
...
bool EteraAPI::parseWait(bool& wait) {
    ...
    if(status == "in-progress") {
        ...
        return setLastError(1, tr("Операцию завершить не удалось, попробуйте повторить запрос позже"));
    else if(status == "success")
        return true;

    return setLastError(1, tr("Неизвестный статус операции"));
}
//----------------------------------------------------------------------------------------------
